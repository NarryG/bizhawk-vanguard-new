...
namespace RTCV.BizhawkVanguard
{
	public static class Hooks
	{
		...
		public static EmuClientApi EmuClient
		{
			get; set;
		}
		public static MemoryApi Memory
		{
			get; set;
		}
		public static EmulationApi Emulation
		{
			get; set;
		}
		public static GameInfoApi GameInfo
		{
			get; set;
		}

		public static void ClearRTCApis()
		{
			EmuClient = null;
			Memory = null;
			Emulation = null;
		...
		public static void SetRTCApis(ApiContainer apis)
		{
			EmuClient = apis.EmuClient as EmuClientApi;
			Memory = apis.Memory as MemoryApi;
			Emulation = apis.Emulation as EmulationApi;
			GameInfo = apis.GameInfo as GameInfoApi;
		}
		...
		public static void CPU_STEP(bool isRewinding, bool isFastForwarding, bool isBeforeStep = false)
		{
			try
			{
				if (disableRTC || GlobalWin.Emulator is NullEmulator)
					return;
		...
		public static void LOAD_GAME_DONE()
		{
			try
			{
				...
				string uppercaseFilename = GameInfo.GetRomName().ToUpper();
				...
				PathEntry pathEntry = GlobalWin.Config.PathEntries[GlobalWin.Game.System, "Savestates"] ??
					GlobalWin.Config.PathEntries[GlobalWin.Game.System, "Base"];
				...
				gameDone[VSPEC.SYSTEMCORE] = BIZHAWK_GET_SYSTEMCORENAME(GlobalWin.Game.System);
				gameDone[VSPEC.SYNCSETTINGS] = BIZHAWK_GETSET_SYNCSETTINGS;
				gameDone[VSPEC.OPENROMFILENAME] = EmuClient.GetRomFullPath();
		...
		public static bool IsAllowedBackgroundInputForm()
		{
			...
			return VanguardConnector.IsUIForm() ||
				(Form.ActiveForm is HexEditor && GlobalWin.Config.HexEditorAllowBackgroundInput);
		...
		public static Bitmap BIZHAWK_GET_SCREENSHOT()
		{
			try
			{
				return EmuClient.ScreenshotToBitmap();
			}
			catch (Exception ex)
			{
				if (VanguardCore.ShowErrorDialog(ex, true) == DialogResult.Abort)
		...
		public static string BIZHAWK_GET_FILESYSTEMGAMENAME()
		{
			try
			{
				...
				return GlobalWin.Game.FilesystemSafeName();
			}
			catch (Exception ex)
			{
				if (VanguardCore.ShowErrorDialog(ex, true) == DialogResult.Abort)
		...
		public static bool BIZHAWK_ISNULLEMULATORCORE()
		{
			try
			{
				...
				return GlobalWin.Emulator is NullEmulator;
			}
			catch (Exception ex)
			{
				if (VanguardCore.ShowErrorDialog(ex) == DialogResult.Abort)
		...
		public static void BIZHAWK_LOADROM(string path)
		{
			try
			{
				EmuClient.OpenRom(path);
			}
			catch (Exception ex)
			{
				if (VanguardCore.ShowErrorDialog(ex, true) == DialogResult.Abort)
		...
		public static void BIZHAWK_OPEN_HEXEDITOR_ADDRESS(MemoryDomainProxy mdp, long address)
		{
			try
			{
				if(mdp?.MD == null)
					return;
				GlobalWin.Tools.Load<HexEditor>();
				GlobalWin.Tools.HexEditor.SetToAddresses(new[] {address},((VanguardImplementation.BizhawkMemoryDomain)(mdp.MD)).MD, WatchSize.Byte);
				GlobalWin.Tools.HexEditor.Show();
			}
			catch (Exception ex)
			{
				if (VanguardCore.ShowErrorDialog(ex, true) == DialogResult.Abort)
		...
		public static Size BIZHAWK_GETSET_MAINFORMSIZE
		{
			get
			{
				return EmuClient.GetMainFormSize();
			}
			set
			{
				EmuClient.SetMainFormSize(value);
			}
		}
		...
		public static Point BIZHAWK_GETSET_MAINFORMLOCATION
		{
			get
			{
				return new Point(EmuClient.Xpos(), EmuClient.Ypos());
			}
			set
			{
				EmuClient.SetMainFormPosition(value);
			}
		}

		public static void BIZHAWK_STARTRECORDAV(string videowritername, string filename, bool unattended)
		{
			EmuClient.StartAV(videowritername, filename, unattended);
		}
		...
		{
			EmuClient.LoadStateRTC(path);
		}

		public static void BIZHAWK_SAVESTATE(string path)
		{
			EmuClient.SaveStateRTC(path);
		}
		...
		public static void BIZHAWK_SET_SYSTEMCORE(string systemName, string systemCore)
		{
			try {
				...
				{
					case "GB":
						GlobalWin.Config.GbAsSgb = false;
						break;
					case "SGB":
						GlobalWin.Config.GbAsSgb = true;
						break;
				}
				GlobalWin.Config.PreferredCores[systemName] = systemCore;
			}
			catch (Exception ex)
			{
				if (VanguardCore.ShowErrorDialog(ex, true) == DialogResult.Abort)
		...
		public static string BIZHAWK_GETSET_SYNCSETTINGS
		{
			get
			{
				SettingsAdapter settable = new SettingsAdapter(EmuClient.Emulator);
		...
		public static MemoryDomainProxy[] GetInterfaces()
		{
			try
			{
				...
				if (EmuClient.Emulator is NullEmulator)
					return new MemoryDomainProxy[] { };

				foreach (MemoryDomain _domain in Memory.DomainList)
		...
		{
			return EmuClient.Emulator.HasDriveLight() && EmuClient.Emulator.AsDriveLight().DriveLightEnabled;
		}
