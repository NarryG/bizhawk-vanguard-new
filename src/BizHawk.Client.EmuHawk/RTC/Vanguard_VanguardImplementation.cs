			...
			try
			{
				...
				switch (message.Type) //Handle received messages here
				{
					...
					case RTCV.NetCore.Commands.Basic.SaveSavestate:
						SyncObjectSingleton.FormExecute(() => { e.setReturnValue(VanguardCore.SaveSavestate_NET(advancedMessage.objectValue as string)); });
						break;
					...
					case RTCV.NetCore.Commands.Basic.LoadSavestate:
					{
						...
						SyncObjectSingleton.FormExecute(() => { e.setReturnValue(VanguardCore.LoadSavestate_NET(path, location)); });
						break;
					}
					...
					case RTCV.NetCore.Commands.Remote.DomainGetDomains:
						SyncObjectSingleton.FormExecute(() => { e.setReturnValue(Hooks.GetInterfaces()); });
						break;
					...
					case RTCV.NetCore.Commands.Remote.KeySetSystemCore:
					{
						...
						SyncObjectSingleton.FormExecute(() => { Hooks.BIZHAWK_SET_SYSTEMCORE(systemName, systemCore); });
					}
						break;
					...
					case RTCV.NetCore.Commands.Emulator.OpenHexEditorAddress:
					{
						...
						var mdp = MemoryDomains.GetProxy(domain, address);
						var realAddress = MemoryDomains.GetRealAddress(domain, address);

						SyncObjectSingleton.FormExecute(() => { Hooks.BIZHAWK_OPEN_HEXEDITOR_ADDRESS(mdp, realAddress); });
					...
					case RTCV.NetCore.Commands.Remote.RenderStart:
						SyncObjectSingleton.FormExecute(() => { BizhawkRender.StartRender_NET(); });
						break;

					case RTCV.NetCore.Commands.Remote.RenderStop:
						SyncObjectSingleton.FormExecute(() => { BizhawkRender.StopRender_NET(); });
						break;
		...
		public class BizhawkMemoryDomain : IMemoryDomain
		{
			...
			public long Size
			{
				get
				{
					...
					if (Hooks.EmuClient.Emulator is N64 && !(Hooks.EmuClient.Emulator as N64).UsingExpansionSlot && Name == "RDRAM")
						return size / 2;
					return size;
				}
			}
			...
			public byte PeekByte(long addr) =>
				...
				addr < MD.Size ? MD.PeekByte(addr) : (byte) 0;
			...
			public byte[] PeekBytes(long addr, int range)
			{
				...
				var returnArray = new byte[range];
			...
			public override string ToString() => MD.Name;
		}
	}
}